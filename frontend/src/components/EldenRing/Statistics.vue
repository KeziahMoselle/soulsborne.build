...
<template>
  <div>
    <FormField v-slot="{ componentField, errors }" name="level">
      ...
      <FormItem class="flex items-center justify-between gap-4">
        <FormLabel class="flex flex-col">
          ...
          <span
            ...
            v-if="stat.softcaps.find(cap => cap.level > Number(value || 0))"
          >
            Next cap:
            {{
              stat.softcaps.find(cap => cap.level > Number(value || 0)).level
            }}
          </span>
        </FormLabel>
        <FormControl>
