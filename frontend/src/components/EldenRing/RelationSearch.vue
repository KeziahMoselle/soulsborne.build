...
const optionsGroups = computed(() =>
  Object.entries(optionsByRelations).map(([relation, options]) => [
    relation,
    options
      .filter(item =>
...
<template>
  <FormField :name="name">
    ...
    >
      <Popover v-model:open="isOpen" @update:open="getAllOptions">
              ...
              >
                <template
                  v-for="(option, index) in (
                    options as PayloadOptionLike[]
                  ).filter(option => option.id !== previewEquipment?.id)"
                ...
                >
                  ...
                  <div
                    class="relative flex flex-col select-none items-center rounded-sm px-2 py-1.5 text-sm opacity-50"
                    v-if="index === options.length - 1"
                    disabled
                  >
                    <p class="m-0">
                      {{
                        optionsByRelations[relation as string].length -
                        options.length
                      }}
                      hidden results.
                    </p>
